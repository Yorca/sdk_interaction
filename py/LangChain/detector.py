...
def detect(apk):
    if not os.path.exists(f"res/trace/{apk}.json") or not os.path.exists(f"res/trace/{apk}.json"):
        return None
    print(apk)
    ...
    for trace in trace_data:
        context_text = ""
        premise_text = ""
        print(trace["event_type"])
        if trace["event_type"] != "method call":
            continue
        if trace["source"] == "sensitive" and not trace["method"] in ["getLastKnownLocation",
