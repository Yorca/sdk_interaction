...
class runtime_context:
    ...
    def get_prompt(self):
        template = """
         ...
         Instructions:
         1. Iterate over the runtime trace, which is a list of dictionaries, and summarize each event.
         ...
         """
        prompt = PromptTemplate(input_variables=["traces"],template=template)
        return prompt

    def build_context(self):
        chain = LLMChain(llm=llm, prompt=self.get_prompt())
        result = chain.run(traces=self.trace)
        self.result = result
