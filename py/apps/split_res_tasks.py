...
with open("downloaded_pkg2.txt", "r") as file:
...
pkgs = []
for filename in os.listdir("../apk_downloader/apks_new"):
    pkgs.append(filename.split("---")[0])

all_pkgs = []
for index, row in total_data.iterrows():
    all_pkgs.append(row[0])
    # if row[0] not in downloads and row[0] not in pkgs:
